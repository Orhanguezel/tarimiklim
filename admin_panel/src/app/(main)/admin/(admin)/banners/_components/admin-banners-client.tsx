      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-gm-gold font-bold text-[10px] tracking-[0.2em] uppercase">Medya & Reklam</span>
          </div>
          <h1 className="font-serif text-4xl text-gm-text">Banners</h1>
          <p className="text-gm-muted text-sm font-serif italic max-w-xl">
            Manage advertising and promotional banners across web and mobile.
          </p>
        </div>
        <div className="flex gap-4 items-center">
          <Button variant="outline" size="sm" onClick={() => query.refetch()} disabled={query.isFetching} className="rounded-full border-gm-border-soft bg-gm-surface/20 hover:bg-gm-surface hover:text-gm-gold font-bold tracking-widest uppercase text-[10px] px-6 h-12">
            <RefreshCcw className={`mr-2 size-4${query.isFetching ? ' animate-spin' : ''}`} />
            Refresh
          </Button>
          <Button size="sm" asChild className="rounded-full bg-gm-gold text-gm-bg hover:bg-gm-gold/90 transition-all font-bold tracking-widest uppercase text-[10px] px-8 h-12 border-0">

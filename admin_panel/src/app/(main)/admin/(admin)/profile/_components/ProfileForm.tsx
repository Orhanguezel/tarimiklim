...
export function ProfileForm() {
  ...
  React.useEffect(() => {
    let mounted = true;
    weatherAdminAlertsApi.getMyTelegramChatId()
      .then((res) => {
        if (!mounted) return;
        const payload = res as { data?: { chat_id?: string | null } };
        setTelegramChatId(payload.data?.chat_id ?? '');
      })
      .catch(() => undefined);
    return () => {
      mounted = false;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await weatherAdminAlertsApi.updateMyTelegramChatId(telegramChatId.trim() || null);
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                Chat ID değerini Telegram’da t.me/userinfobot üzerinden öğrenebilirsiniz.
              </p>
            </div>
          </div>
        </CardContent>

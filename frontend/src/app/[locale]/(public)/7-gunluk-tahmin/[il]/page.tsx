...
export async function generateMetadata({
  ...
  if (!province) return {};
  return buildCityMetadata('7-gunluk-tahmin', locale, province);
}

export default async function YediGunlukCityPage({
  params,
}: {
  params: Promise<{ locale: string; il: string }>;
}) {
  ...
  return (
    <section className="panel-shell">
      <CityWeatherView variant="7-gunluk-tahmin" locale={locale} province={province} />
    </section>
  );
}
